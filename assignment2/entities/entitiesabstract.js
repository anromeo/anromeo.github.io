...
LivingEntity.prototype.draw = function (ctx) {
    ...
    if (this.movingAnimation && this.currentAction === "chasing") { 
        this.movingAnimation.drawFrameRotate(this.game.clockTick, ctx, this.x - this.radius, this.y - this.radius, this.angle);
    } else if (this.attackAnimation && this.currentAction === "attacking") {
        this.attackAnimation.drawFrameRotate(this.game.clockTick, ctx, this.x - this.radius, this.y - this.radius, this.angle);        
    } else if (this.deathAnimation && this.currentAction === "dying") {
        this.deathAnimation.drawFrameRotate(this.game.clockTick, ctx, this.x - this.radius, this.y - this.radius, this.angle);                
    }
        // ctx.beginPath();
        // ctx.strokeStyle = "green";
        // ctx.lineWidth = 1;
        // ctx.arc(this.x, this.y, this.radius + this.rangeOfAttack, 0, Math.PI * 2, false);
        // ctx.closePath();
        // ctx.stroke();

    if (this.currentAction === "growing") {
        this.attackAnimation.drawFrameRotate(this.game.clockTick, ctx, this.x - this.radius, this.y - this.radius, this.angle);
        ...
        if (this.game.entities.length > 1) {
            ...
            ctx.lineWidth = 1;
            ctx.arc(this.x, this.y, this.radius + this.rangeOfAttack + this.halo, 0, Math.PI * 2, false);
            ctx.arc(this.x, this.y, this.radius + this.rangeOfAttack + this.halo - 2, 0, Math.PI * 2, false);
            ctx.arc(this.x, this.y, this.radius + this.rangeOfAttack + this.halo - 4, 0, Math.PI * 2, false);
            ctx.closePath();
            ctx.stroke();
        }
    }
